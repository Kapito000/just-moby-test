...
{
	public sealed class CubesListView : MonoBehaviour, ICubesListView
	{
		[SerializeField] Transform _itemsParent;
		[ReadOnly]
		[SerializeField] List<CubeListItemView> _items;

		[Inject] ICubesListViewFactory _factory;
		...
		public void UpdateList(ICubeConfigDataProvider[] configs)
		{
			AdjustItemsCount(configs.Length);
			AdjustItemsViews(configs);
		}

		void AdjustItemsViews(ICubeConfigDataProvider[] characters)
		{
